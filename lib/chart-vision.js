...
// single-chart branch, since the watcher supplies one auto-generated Daily
// chart, not a multi-timeframe screenshot set). If you update the prompt in
// index.html, mirror the change here too -- same drift risk noted in
// cpr-engine.js applies.
//
...
function buildPrompt(symLabel, mode) {
  ...
  // Single-chart branch, verbatim from index.html: the watcher supplies one
  // auto-generated Daily chart, not a multi-timeframe screenshot set.
  const mtfManifest = 'You have ONE chart: the Daily timeframe. Higher-timeframe context is NOT supplied -- infer it only where the chart genuinely supports it, say so in stepByStep, and score Trend Alignment conservatively.\n\n';
...
async function callChartVision(symLabel, mode, imageBase64, mediaType) {
